...
Lives in its own `monitoring` namespace, deployed via the same ArgoCD `Application` and `deployment` branch as everything else — no separate pipeline. Loki and Promtail are the one exception: they're installed via Helm directly in the EC2 boot script (`user_data`) rather than through ArgoCD, since they're infra tooling rather than app-adjacent manifests — see `docs/SETUP.md`.
...
- **Promtail** (DaemonSet, one per node) — tails container logs from `/var/log/pods` on the node and ships them to Loki, tagged with pod/namespace/container labels automatically
- **Loki** — stores and indexes the logs Promtail ships, queryable from Grafana
...
Unlike Prometheus's pull-based scraping, Promtail is push-based: it runs as a DaemonSet so there's one instance per node, watches `/var/log/pods` directly on that node's filesystem (where containerd/k3s already writes every container's stdout/stderr), and pushes new log lines to Loki as they appear. No annotations needed on app pods — if a pod is running on the node, its logs get picked up automatically.
...
**Does kube-state-metrics, Loki, or Promtail need the same fix?** No. All three `NetworkPolicy` objects in this repo (`frontend-policy`, `backend-policy`, `postgres-policy`) live in the `novus` namespace and only select pods there. There is no `NetworkPolicy` selecting anything in `monitoring`, which means Kubernetes treats that namespace as fully open by default — no policy means no restriction, not the other way around. Prometheus can reach kube-state-metrics, and Promtail can reach Loki, with zero extra config.

Worth knowing the flip side: since nothing in `monitoring` is locked down, Prometheus/Grafana/kube-state-metrics/Loki are all reachable by anything else on the cluster network, not just each other — and Loki itself has no authentication in front of it (`auth_enabled: false`), so anything in-cluster can push or query logs. A `default-deny` policy for that namespace would be the natural hardening step later — not needed for a solo project right now.
...
- **Loki:** never accessed directly either — query it through Grafana's **Explore** view, not its own UI (it doesn't have one). Pick the Loki datasource, then query by namespace, e.g. `{namespace="novus"}`, to see live logs from the app without SSHing in for `kubectl logs`.

Both the **Prometheus** and **Loki** datasources in Grafana are pre-provisioned (`grafana.yaml`'s `grafana-datasources` ConfigMap) — no manual "add data source" step, both are there the moment you log in.
...
- **Alertmanager** — metrics and logs are both collected and visible, but nothing pages/notifies on thresholds yet.
- **Durable log retention** — Loki's storage is a PVC tied to the instance's local disk, same as Postgres. A `terraform destroy` wipes log history along with the database. Fine for a solo project; a real fix would mean backing Loki with S3 instead of local filesystem storage.